...
test('task creation queues timeline synchronization job with related ids', function () {
    Queue::fake();
    ...
    Queue::assertPushed(
        ...
        fn (SyncRelationshipTimelineJob $job): bool => $job->userId === $user->id
            && $job->companyIds === [$company->id]
            && $job->contactIds === [$contact->id],
    );
});

test('task creation auto-fills company from contact and syncs follow-up dates', function () {
    $user = User::factory()->create();
...
test('task creation auto-fills contact and company from related activity', function () {
    $user = User::factory()->create();
...
test('activity creation auto-fills company from contact and syncs last contacted dates', function () {
    $user = User::factory()->create();
...
test('timeline sync recalculates oldest and newest relations after task reassignment and deletion', function () {
    $user = User::factory()->create();
...
test('last contacted sync uses latest completed record across activities and tasks', function () {
    $user = User::factory()->create();
